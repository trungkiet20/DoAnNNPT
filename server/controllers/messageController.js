...
exports.sendMessage = async (req, res) => {
  try {
    ...
    // Real-time: Emit to landlord
    socket.emitToUser(room.landlordId, 'receive_message', {
      roomId,
      tenantId,
      message
    });
...
exports.replyMessage = async (req, res) => {
  try {
    ...
    // Real-time: Emit to tenant
    socket.emitToUser(tenantId, 'receive_message', {
      roomId,
      landlordId,
      message
    });
...
exports.sendInvitation = async (req, res) => {
  try {
    ...
    // Real-time: Emit notification and system message to tenant
    socket.emitToUser(tenantId, 'receive_invitation', {
      invitation: full
    });
    socket.emitToUser(tenantId, 'receive_message', {
      roomId,
...
exports.respondInvitation = async (req, res) => {
  try {
    ...
    // Real-time: Emit status update and system message to landlord
    socket.emitToUser(inv.landlordId, 'invitation_status_updated', {
      invitationId: id,
      status,
      message: systemMsg
    });
    socket.emitToUser(inv.landlordId, 'receive_message', {
      roomId: inv.roomId,
      tenantId: inv.tenantId,
      message: systemMsg
    });
