...
window.addEventListener('socket_new_message', (e) => {
    ...
    if (user.role === 'Tenant' && typeof tenant !== 'undefined') {
        if (tenant.residentTab === 'notifications') {
            tenant.loadResidentData().then(() => tenant.renderPage());
        }
    } else if (user.role === 'Landlord' && typeof landlord !== 'undefined') {
        // If landlord is in the inbox view, refresh it
        // Note: You might need to check if the landlord is currently viewing messages
        landlord.loadInbox?.(); 
    }
});
